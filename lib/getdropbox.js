...
function syncFolder() {
	...
	if (process.env.DROPBOX_ACCESSTOKEN !== undefined && process.env.DROPBOX_FOLDER !== undefined) {
		var dropboxfolder = process.env.DROPBOX_FOLDER;
					...
					if (response.error_summary === undefined) {
							for (var i = 0; i < response.entries.length; i++) {
								if (response.entries[i][".tag"] === "folder") {
									syncSubFolder(response.entries[i].path_lower)
								}
							...
							// if (global.content_hash_list.length === 0) {
...
function syncSubFolder(subfolder) {

	global.content_hash_list[subfolder] = [];
	global.path_lower_list[subfolder] = [];

	var formquery = '{"path":"' + subfolder +'"}';
			...
			if (response !== undefined) {
				if (response.error_summary === undefined) {
						for (var i = 0; i < response.entries.length; i++) {
							...
							if (/.jpg$/ig.test(response.entries[i].path_lower)) {
								global.content_hash_list[subfolder].push(response.entries[i].content_hash + ".jpg");
								global.path_lower_list[subfolder].push(response.entries[i].path_lower);
								checkIfFileExist(response.entries[i].content_hash + ".jpg",response.entries[i].path_lower)
							}
						}
						if (global.content_hash_list[subfolder].length === 0) {
							global.content_hash_list[subfolder].push("default.png")
							console.log("lenght==0");
						}
				}
				if (response.error_summary !== undefined) {
