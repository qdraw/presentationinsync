				...
				if (response !== undefined) {
					if (response.error_summary === undefined) {
							for (var i = 0; i < response.entries.length; i++) {
								...
								if (/.jpg$/ig.test(response.entries[i].path_lower) || /config.json$/ig.test(response.entries[i].path_lower)) {
...
function pushContentVar(relativeFilePath) {
	...
	if ( /.jpg$/ig.test(relativeFilePath) ) {
...
// uploadFile("/data/git/presentationinsync/images/4. dashboard 2.jpg","/presentationinsync/temp.jpg")
